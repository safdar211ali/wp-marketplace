 ...
 */
function marketplace_customize_register( $wp_customize ) {
	$color_scheme = marketplace_get_color_scheme();
	...
	if ( isset( $wp_customize->selective_refresh ) ) {
		$wp_customize->selective_refresh->add_partial( 'blogname', array(
			...
			'render_callback' => 'marketplace_customize_partial_blogname',
		) );
		$wp_customize->selective_refresh->add_partial( 'blogdescription', array(
			'selector' => '.site-description',
			'container_inclusive' => false,
			'render_callback' => 'marketplace_customize_partial_blogdescription',
		) );
	}
	...
	$wp_customize->add_setting( 'color_scheme', array(
		'default'           => 'default',
		'sanitize_callback' => 'marketplace_sanitize_color_scheme',
		'transport'         => 'postMessage',
	) );

	$wp_customize->add_control( 'color_scheme', array(
		'label'    => __( 'Base Color Scheme', 'marketplace' ),
		'section'  => 'colors',
		'type'     => 'select',
		'choices'  => marketplace_get_color_scheme_choices(),
		'priority' => 1,
	) );
	...
	$wp_customize->add_control( new WP_Customize_Color_Control( $wp_customize, 'sidebar_textcolor', array(
		'label'       => __( 'Header and Sidebar Text Color', 'marketplace' ),
		'description' => __( 'Applied to the header on small screens and the sidebar on wide screens.', 'marketplace' ),
		'section'     => 'colors',
	) ) );
	...
	$wp_customize->add_control( new WP_Customize_Color_Control( $wp_customize, 'header_background_color', array(
		'label'       => __( 'Header and Sidebar Background Color', 'marketplace' ),
		'description' => __( 'Applied to the header on small screens and the sidebar on wide screens.', 'marketplace' ),
		'section'     => 'colors',
	) ) );

	// Add an additional description to the header image section.
	$wp_customize->get_section( 'header_image' )->description = __( 'Applied to the header on small screens and the sidebar on wide screens.', 'marketplace' );
}
add_action( 'customize_register', 'marketplace_customize_register', 11 );

/**
 * Render the site title for the selective refresh partial.
 *
 * @since Twenty Fifteen 1.5
 * @see marketplace_customize_register()
 *
 * @return void
 */
function marketplace_customize_partial_blogname() {
	bloginfo( 'name' );
}
 ...
 * @return void
 */
function marketplace_customize_partial_blogdescription() {
 ...
 *
 * Can be filtered with {@see 'marketplace_color_schemes'}.
 *
 ...
 */
function marketplace_get_color_schemes() {
	/**
	 ...
	 * }
	 */
	return apply_filters( 'marketplace_color_schemes', array(
		'default' => array(
			'label'  => __( 'Default', 'marketplace' ),
			'colors' => array(
				'#f1f1f1',
				'#ffffff',
				'#ffffff',
...
if ( ! function_exists( 'marketplace_get_color_scheme' ) ) :
/**
 ...
 */
function marketplace_get_color_scheme() {
	$color_scheme_option = get_theme_mod( 'color_scheme', 'default' );
	$color_schemes       = marketplace_get_color_schemes();
...
endif; // marketplace_get_color_scheme

if ( ! function_exists( 'marketplace_get_color_scheme_choices' ) ) :
/**
 ...
 */
function marketplace_get_color_scheme_choices() {
	$color_schemes                = marketplace_get_color_schemes();
...
endif; // marketplace_get_color_scheme_choices

if ( ! function_exists( 'marketplace_sanitize_color_scheme' ) ) :
/**
 ...
 */
function marketplace_sanitize_color_scheme( $value ) {
	$color_schemes = marketplace_get_color_scheme_choices();
...
endif; // marketplace_sanitize_color_scheme
 ...
 */
function marketplace_color_scheme_css() {
	$color_scheme_option = get_theme_mod( 'color_scheme', 'default' );
	...
	$color_textcolor_rgb         = marketplace_hex2rgb( $color_scheme[3] );
	$color_sidebar_textcolor_rgb = marketplace_hex2rgb( $color_scheme[4] );
	$colors = array(
	...
	$color_scheme_css = marketplace_get_color_scheme_css( $colors );

	wp_add_inline_style( 'marketplace-style', $color_scheme_css );
}
add_action( 'wp_enqueue_scripts', 'marketplace_color_scheme_css' );
 ...
 * @since Twenty Fifteen 1.0
 */
function marketplace_customize_control_js() {
	...
	wp_localize_script( 'color-scheme-control', 'colorScheme', marketplace_get_color_schemes() );
}
add_action( 'customize_controls_enqueue_scripts', 'marketplace_customize_control_js' );
 ...
 * @since Twenty Fifteen 1.0
 */
function marketplace_customize_preview_js() {
	wp_enqueue_script( 'marketplace-customize-preview', get_template_directory_uri() . '/js/customize-preview.js', array( 'customize-preview' ), '20141216', true );
}
add_action( 'customize_preview_init', 'marketplace_customize_preview_js' );
 ...
 */
function marketplace_get_color_scheme_css( $colors ) {
 ...
 * @since Twenty Fifteen 1.0
 */
function marketplace_color_scheme_css_template() {
	$colors = array(
	...
	?>
	<script type="text/html" id="tmpl-marketplace-color-scheme">
		<?php echo marketplace_get_color_scheme_css( $colors ); ?>
	</script>
	<?php
}
add_action( 'customize_controls_print_footer_scripts', 'marketplace_color_scheme_css_template' );
